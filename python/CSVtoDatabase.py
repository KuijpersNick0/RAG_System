...
# print(json.dumps(chapters["EventListeners"]["Sams.EventListener.MQTT.Broker"], indent=2))
# print(chapters)
...
chroma_client.delete_collection(name="Sams_Documentation") 
# collection = chroma_client.get_collection(name="Sams_Documentation_v2")
collection = chroma_client.create_collection(name="Sams_Documentation")
 

def upload_to_vector_database(collection, payload): 
    # Sleep to avoid rate limiting
    time.sleep(30)
    collection.add(
        embeddings=[payload["embedded_data"]],
        metadatas=[payload["metadata"]],
        ids=[payload["metadata"]["id"]]
    )
    print("Uploaded to Chroma {}".format(payload["metadata"]["id"]))

# Iterate through the nested dictionaries
for chapter_name, chapter_data in chapters.items():
    for connector_name, connector_data in chapter_data.items(): 

        # print("Processing connector: ", connector_name)
        # print("Description: ", connector_data.get("Description", "")) 
        # print("Text: ", json.dumps((connector_data), indent=2))
        
        # First : Extract metadata
        metadata = {
            "is_reference": True,
            "external_source_name": "Sams_Documentation",
            "id": connector_name,
            "description": connector_data.get("Description", ""),
            "text": json.dumps((connector_data), indent=2),
            "additional_metadata": "chapter_name: {}".format(chapter_name)
        }
        
        # Prepare text for embedding : I'm keeping chapter, connector name and description
        # Too much embedding will resolve in difficulty to find the right connector with semantic search
        ...
        # Get embeddings
        embedded_data = get_embedding(text_to_embed, engine='text-embedding-ada-002')
        ...
        # print(payload["embedded_data"])
        
        # print(payload)
        

        # Upload payload to vector database 
        upload_to_vector_database(collection, payload)
